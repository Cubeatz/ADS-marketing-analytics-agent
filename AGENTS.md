...
5. 连续 3 次失败后立即停止该次日报流程，不生成假数据报告。
6. 失败时用用户能理解的话告知：哪个平台授权过期、需要去哪个后台或 IDE MCP 面板重新授权、日报已停止。
7. 如果已配置飞书 webhook，应推送授权失败提醒；未配置飞书时，在 `reports/{date}/auth-failed.md` 写一份失败说明。
...
3. 对启用平台执行授权健康检查；发现过期按“三次重试”规则处理。
4. 读取启用平台，通过 MCP 只读拉数，写入 `temp/raw/{date}/{platform}/{category}/`。
5. 清洗到 `temp/processed/`，日志写入 `temp/logs/`。
6. 分析并生成 `reports/`。
7. 按 `delivery.mode` 投递本地 Word、飞书或 Markdown。
